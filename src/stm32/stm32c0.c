// Code to setup clocks on stm32c0
//
...
lookup_clock_line(uint32_t periph_base)
{
    ...
    if (periph_base == SYSCFG_BASE)
        return (struct cline){.en=&RCC->APBENR2,.rst=&RCC->APBRSTR2,.bit=1<<0};
...
clock_setup(void)
{
    ...
    // 2. Configure Flash latency - Required for 48MHz operation
    // One wait state is required for 48MHz operation.
    FLASH->ACR = (FLASH->ACR & ~FLASH_ACR_LATENCY) | FLASH_ACR_LATENCY_0;

    // 3. Wait for HSI to be ready
    while (!(RCC->CR & RCC_CR_HSIRDY))
        ;

    // 4. Select HSI as system clock
    RCC->CFGR &= ~RCC_CFGR_SW;

    // 5. Wait for clock switch confirmation
    while ((RCC->CFGR & RCC_CFGR_SWS_Msk) != RCC_CFGR_SWS_HSI)
        ;

    // Optional: Ensure peripheral buses (APB/AHB) are at full speed (DIV1)
    // Most Klipper builds default these to 0 (No division) anyway.
